...
function kittymachine_settings_init()
{
	...
	add_settings_field(
		...
		array(
			...
			'isEnabled' => 'kittymachine_field_auto_refresh',
			'class'             => 'kittymachine_row',
			'kittymachine_custom_data' => 'custom',
		)
	);
...
add_action('rest_api_init', 'kittymachine_settings_init');
...
{
?>
	<p id="<?php echo esc_attr($args['id']); ?>"><?php esc_html_e('Lets you toggle automatic update every 5 seconds.', 'kittymachine'); ?></p>
<?php
}

/**
 * Automatic refresh field callbakc function.
 *
...
function kittymachine_field_auto_refresh_frequency_cb($args)
{
	...
	$display = $options[$args['isEnabled']];
?>
